    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900 text-white">
      <nav className="flex items-center justify-between px-6 py-5 max-w-6xl mx-auto">
        <div className="flex items-center gap-2 text-xl font-bold">
          <Nfc className="text-purple-400" size={28} /> RelayCrd
        </div>
        <div className="flex gap-3">
          <Link href="/login" className="px-4 py-2 text-sm text-slate-300 hover:text-white transition-colors">Sign In</Link>
          <Link href="/signup" className="px-4 py-2 text-sm bg-purple-600 hover:bg-purple-500 rounded-lg font-medium transition-colors">Get Started</Link>
        </div>
      </nav>
      ...
      <section className="text-center px-6 pt-20 pb-32 max-w-4xl mx-auto">
        <div className="inline-flex items-center gap-2 bg-purple-900/50 border border-purple-700/50 rounded-full px-4 py-1.5 text-sm text-purple-300 mb-8">
          <Zap size={14} /> The future of networking is here
        </div>
        <h1 className="text-5xl sm:text-6xl font-extrabold mb-6 leading-tight">
          Your business card,<br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">just one tap away</span>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <Link href="/pricing" className="px-8 py-3.5 border border-slate-700 hover:border-slate-500 rounded-xl font-semibold text-lg transition-colors text-slate-300">
            See Pricing
          </Link>
        </div>
      </section>
      ...
      <section className="text-center px-6 pb-24">
        <div className="max-w-xl mx-auto bg-gradient-to-br from-purple-900/60 to-pink-900/30 border border-purple-700/40 rounded-3xl p-10">
          ...
          <p className="text-slate-400 mb-8">Free forever. Upgrade when you grow.</p>
          <div className="flex gap-3 justify-center">
            ...
            <Link href="/pricing" className="inline-block px-8 py-3.5 border border-slate-600 hover:border-slate-400 rounded-xl font-semibold transition-colors text-slate-300">
              View Pricing
            </Link>
          </div>
        </div>
      ...
      <footer className="text-center text-slate-600 text-sm pb-8">
        © 2024 RelayCrd. Built for the future of networking.
      </footer>
    </div>
  );
}
